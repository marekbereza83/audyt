...
- **Zrzuty długich stron są cięte na kafelki — otwieraj wszystkie.** `fullPage: true` na długiej stronie daje obraz, którego model nie odczyta: skalowanie do 1568 px na dłuższym boku sprowadza pas 1920×11111 do **271 px szerokości**, a mobile 375×20179 do **29 px**. Zmierzone na paczce Katowice+Gliwice: 6 z 14 zrzutów desktop poniżej 550 px, wszystkie mobile długich stron w przedziale 29–48 px — czyli ocena mobile była tam fikcją. `zrzucKafelki()` tnie stronę na kafelki o wysokości dobranej **do szerokości** (stała wysokość nie wystarcza: 4000 px daje czytelne 753 px przy 1920 px, ale 147 px przy 375 px). Pierwszy kafelek zachowuje starą nazwę (`screenshot-desktop.png`), kolejne dostają `-2`, `-3`. Przy stronach dłuższych niż limit kafelków ostatni kafelek to **stopka** (rok w copyright, kredyt agencji to udokumentowane markery Kroku 0), więc gubi się tylko środek. Sterowanie: `SCREENSHOT_MAX_H` (domyślnie 4000), `SCREENSHOT_MIN_W` (600).
- **Lighthouse chodzi w procesie potomnym (`lighthouse-worker.js`), nie pod muteksem.** Kolizja globalnych `performance.mark` dotyczy wyłącznie wspólnego procesu, więc `fork()` znosi powód serializacji. Zmierzone: 35,9 s na firmę; 4 pomiary równolegle 62,3 s zamiast 143,6 s zserializowanych (**2,3×**). Wynik wraca przez **IPC, nie stdout** — Lighthouse i chrome-launcher piszą po drodze własne komunikaty, więc stdout nie jest wiarygodnym kanałem na JSON. Limity: `LIGHTHOUSE_ROWNOLEGLE` (4) i `LIGHTHOUSE_TIMEOUT_MS` (120 s, żeby zawieszona strona nie zablokowała paczki). **Uwaga na realny zysk w paczce:** przy domyślnym `FIRECRAWL_ROWNOLEGLE=2` to slot audytu, a nie Lighthouse, jest teraz ogranicznikiem — pełny zysk z równoległości wymagałby rozdzielenia etapów (Firecrawl trzyma limit 2 równoległych, Playwright i Lighthouse już nie).
